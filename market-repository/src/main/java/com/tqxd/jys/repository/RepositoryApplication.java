...
public class RepositoryApplication extends AbstractVerticle {
  ...
  public void start(Promise<Void> startPromise) throws Exception {
    ...
    kLineRepository.open(vertx, config())
        // 将k线数据注册到k线仓库同步器
      ...
      router.get("/market/history/kline").handler(ctx -> {
        ...
        kLineRepository.query(symbol, Period.valueOfSymbol(period), lastDay, now, ar -> {
          if (ar.succeeded()) {
            ...
            result.put("ch",
                String.format("market.%s.kline.%s", symbol, Objects.requireNonNull(Period.valueOfSymbol(period))));
            result.put("status", "ok");
            result.put("data", ar.result());
            ctx.response().putHeader("content-type", "application/json")
                .putHeader("Access-Control-Allow-Origin", "*")
                .putHeader("Access-Control-Allow-Methods", "GET, POST, OPTIONS, PUT")
                .putHeader("Access-Control-Allow-Credentials", "true").end(Json.encodePrettily(result));
          } else {
      ...
      router.get("/v1/common/symbols").handler(ctx -> {
      ...
      router.route()
          .handler(CorsHandler.create("*").allowedMethod(io.vertx.core.http.HttpMethod.GET)
              .allowedMethod(HttpMethod.PUT).allowedMethod(io.vertx.core.http.HttpMethod.POST)
      ...
      vertx.createHttpServer().requestHandler(router).exceptionHandler(Throwable::printStackTrace)
          .listen(8889, "0.0.0.0").onComplete(ar -> {
        if (ar.succeeded()) {
        ...
        .onFailure(Throwable::printStackTrace).onComplete(startPromise);
  ...
  public void stop(Promise<Void> stopPromise) throws Exception {
    ...
    MessageBusFactory.bus().unSubscribe(Topic.KLINE_TICK_TOPIC, msgBusRegistryId, ar -> {
      if (ar.succeeded()) {
