...
import com.tqxd.jys.core.kline.cmd.*;
...
public class InMemKLineRepository implements KLineRepository {
  ...
  /**
   * 名称 -> timeLine 映射的索引
   * 用数组和map作为索引，因为需要频繁遍历，避免经常遍历映射导致频繁创建 {@link Iterator} 迭代器对象
   */
  ...
  private void doTickKLines() {
    for (int i = 0; i < size; i++) {
      ...
      if (timeLine != null) {
        ...
        if (aggregate != null) {
          onAutoAggregate(new AutoAggregateResult(timeLine.meta().snapshot(), aggregate));
        }
      }
    }
  }
  ...
  private void doRestore(RestoreSnapshotCmd cmd) {
    ...
    getOrCreate(meta.getSymbol(), meta.getPeriod())
      .restoreWithSnapshot(cmd.getSnapshot(), ar -> {
        if (ar.succeeded()) {
          this.onAutoAggregate(ar.result());
  ...
  private void onAutoAggregate(AutoAggregateResult rs) {
    for (KLineRepositoryListener listener : listeners) {
      vertx.executeBlocking(prom -> {
        try {
          listener.onAutoAggregate(rs);
        } catch (Exception e) {
          e.printStackTrace();
        } finally {
          prom.complete();
  ...
  private void onAppendFinished(AppendTickResult rs) {
    for (KLineRepositoryListener listener : listeners) {
      vertx.executeBlocking(prom -> {
        try {
          listener.onAppendFinished(rs);
        } catch (Exception e) {
          e.printStackTrace();
        } finally {
          prom.complete();
